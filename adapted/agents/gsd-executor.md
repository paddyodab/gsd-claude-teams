...
<final_commit>
```bash
node ~/.claude/get-shit-done/bin/gsd-tools.cjs commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md "$(node ~/.claude/get-shit-done/bin/gsd-tools.cjs state-path)" .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
