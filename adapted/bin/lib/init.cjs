...
const { resolveStatePath, resolveCurrentAgentIdPath } = require('./teams.cjs');
...
function cmdInitResume(cwd, raw) {
  ...
  try {
    interruptedAgentId = fs.readFileSync(resolveCurrentAgentIdPath(path.join(cwd, '.planning')), 'utf-8').trim();
  } catch {}
...
function cmdInitProgress(cwd, raw) {
  ...
  let pausedAt = null;
  try {
    const state = fs.readFileSync(resolveStatePath(path.join(cwd, '.planning')), 'utf-8');
